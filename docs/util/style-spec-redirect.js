...
function redirect(location) {
    ...
    if (expressionExceptions.indexOf(location.hash.replace('#', '')) > -1) {
        return `/ja-maplibre-gl-js-docs/style-spec/expressions/${location.hash}`;
    }
    ...
    if (location.hash === '#other-filter') {
        return `/ja-maplibre-gl-js-docs/style-spec/other/#other-filter`;
    }
    ...
    if (
        ...
        return `/ja-maplibre-gl-js-docs/style-spec/layers/${location.hash}`;
    }
    ...
    hash.splice(0, 1);
    return `/ja-maplibre-gl-js-docs/style-spec/${page}/${
